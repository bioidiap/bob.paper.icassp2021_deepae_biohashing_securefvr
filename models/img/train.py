...
def AE_loss(decoded, native):
    ...
    MSE = nn.MSELoss()
    BCE = nn.BCELoss()
    return 0.25*MAE(decoded, native)+ MSE(decoded, native) #BCE(decoded, native)
                ...
                if i != k:
                    ...
                    for j in np.random.randint(all_feats.shape[0], size=10):
                        ...
                        if all_client_ids[i] !=  all_client_ids[j]:    
                            ...
                            distance_anchor_positive, distance_anchor_negative, distance_positive_negative, embeding_triplet_loss = my_triplet_loss(embedded_anchor, embedded_positive, embedded_negative)
...
with open('mylog.csv','w') as f:
    f.write('epoch, iteration, lr, total loss, anchor_MAE, pos_MAE, neg_MAE, anchor_MSE, pos_MSE, neg_MSE, triplet, p/n, a/p, a/n \n')
    ...
    for data in data_gen(all_feats_train, all_client_ids_train, batch_size):
        ...
        if iteration % 100 == 0:            
            with open('mylog.csv','a') as f:
                f.write('{}, {}, {}, {:.4f}, {:.4f}, {:.4f}, {:.4f}, {:.4f}, {:.4f}, {:.4f}, {:.4f}, {:.4f}, {:.4f}, {:.4f}, {:.4f}\n'.format(epoch+1, iteration, all_itrations, optimizer.param_groups[0]["lr"], total_loss.data.item(),\
                        anchor_MAE.item(), positive_MAE.item(), negative_MAE.item(),\
                        anchor_MSE.item(), positive_MSE.item(), negative_MSE.item(),\
                        embeding_triplet_loss.item(),\
...
#                     ))
        ...
        if iteration==train_steps_per_epoch:
            break
    print('epoch [{}/{}], total loss:{:.4f}, anchor_MAE:{:.4f}, pos_MAE:{:.4f}, neg_MAE:{:.4f}, anchor_MSE:{:.4f}, pos_MSE:{:.4f}, neg_MSE:{:.4f}, triplet:{:.4f}, p/n:{:.4f}, a/p:{:.4f}, a/n:{:.4f}'\
              .format(epoch+1, num_epochs, total_loss.data.item(),\
                      anchor_MAE.item(), positive_MAE.item(), negative_MAE.item(),\
                      anchor_MSE.item(), positive_MSE.item(), negative_MSE.item(),\
                      embeding_triplet_loss.item(),\
                      distance_positive_negative.item(), distance_anchor_positive.item(), distance_anchor_negative.item()))
